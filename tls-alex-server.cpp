...
#define LAPTOP_COMMON_NAME "laptop.epp.com"
...
void handleCommand(void *conn, const char *buffer)
{
    ...
    switch(cmd)
    {
        ...
        case 'z':
        case 'Z':
            commandPacket.command = COMMAND_END;
            uartSendPacket(&commandPacket);
            break;
        case 'w':
        case 'W':
        ...
        case 'e':
        case 'E':
            commandPacket.command = COMMAND_E;
            uartSendPacket(&commandPacket);
            break;
        default:
            printf("Bad command\n");
...
int main()
{
    ...
    createServer(ALEX_PRIVATE_KEY, CERTIFICATE, SERVER_PORT, worker, CA_CERT_NAME, LAPTOP_COMMON_NAME, true); 
